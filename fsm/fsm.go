...
func (f *FSM) CurrentMode() string {
	f.mu.RLock()
	defer f.mu.RUnlock()
	if f.currentMode == nil {
		return ""
	}
	return f.currentMode.ID
}
func (f *FSM) RoundNum() int { f.mu.RLock(); defer f.mu.RUnlock(); return f.roundNum }
func (f *FSM) State() *config.StateConfig {
	f.mu.RLock()
	defer f.mu.RUnlock()
...
func (f *FSM) Transition(to string, action config.Action) {
	...
	if f.OnTransition != nil {
		f.OnTransition(from, to, action)
	}
}
func (f *FSM) SetMode(id string) {
	f.mu.Lock()
	m := f.gameModes[id]
	f.currentMode = m
	f.mu.Unlock()
	if m != nil && f.OnModeSet != nil {
		f.OnModeSet(m)
	}
}
func (f *FSM) ResolveAction(stateID string) config.Action {
	f.mu.RLock()
	defer f.mu.RUnlock()
	if f.currentMode != nil {
		if o, ok := f.currentMode.ActionOverrides[stateID]; ok {
			return o
		}
	}
	if s := f.states[stateID]; s != nil {
		return s.Action
	}
